...
export default function RatingsBlock({ p } : Props) {
  function ratingNumBlock(rating: number, label: string, additional: number = -1) {
    return <div style={{...styles.col, gap: '0', padding: '8px 8px 4px', 
      border: '1.5px solid gray', borderRadius: '8px', textAlign: 'center',
      boxShadow: '-4px 4px 4px lightgray'
    }}>
      <div style={{ margin: '0', padding: '0', fontSize: '0.7rem', lineHeight: '0.7rem', fontWeight: 'bold', opacity: 0.7 }}>{label}</div>
      { rating === 0 ? 
      ...
      <div style={{...styles.row, gap: '40px', flexWrap: 'wrap', rowGap: '20px' }}>
        <div style={{...styles.col, justifyContent: 'left'}}>
          ...
          <div style={{...styles.row, flexWrap: 'wrap' }}>
            ...
            { ratingNumBlock(ratings.scoring.free_throw, "FT") }
            { ratingNumBlock(ratings.scoring.draw_foul, "DRAW FOUL") }
          </div>
        </div>
        <div style={{...styles.col, justifyContent: 'left'}}>
          <div><b>DEFENSE</b></div>
        ...
        <div style={{...styles.col, justifyContent: 'left'}}>
          <div><b>TENDENCIES</b></div>
          <div style={{...styles.row, flexWrap: 'wrap' }}>
              { ratingNumBlock(ratings.scoring.zones.restricted_area.tendency, "RESTRICTED AREA") }
              { ratingNumBlock(ratings.scoring.zones.paint_non_ra.tendency, "PAINT NON-RA") }
              { ratingNumBlock(ratings.scoring.zones.mid_range.tendency, "MID RANGE") }
