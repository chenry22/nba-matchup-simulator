...
export function getRatings (p: Player) {
    const minsAdjust = Math.min(1, Math.min(20, p.mins) / 20 * Math.min(60, p.gp) / 40);
    ...
    const threeBoost = Math.min(1, Math.max(0.01, p.scoring.fg3a_per100 - 2) / 6) * Math.max(1, 1 + (p.scoring.fg3a_per100 - 8) / 32);
    ...
    return {
        ...
        scoring: {
            overall: (p.scoring.pts_per100 - 8) * 100 / 32,
            efficiency: Math.max(0, p.scoring.efg_pct - 0.28) * 100 / 0.4,
            ...
            zones: {
                ...
                above_break_3: {
                    skill: getShotRating(p.scoring.zones.above_break_3.ast_fg_pct, p.scoring.zones.above_break_3.uast_fg_pct, 0.18, 0.48)
        ...
        defense: {
            overall: (p.defense.overall - 10) * -100 / 20 ,
            restricted_area: Math.min(0.01, p.defense.restricted_area.pct_plusmin - 8 + 0.001) * -100 / 16 * minsAdjust, 
            ...
            three: Math.min(0.01, p.defense.three.pct_plusmin - 6 + 0.001) * -100 / 13 * minsAdjust,

            stl: Math.max(0.01, p.defense.stl_per100 - 0.4) * 100 / 2.6, 
            blk: Math.min(100, Math.max(0.01, p.defense.blk_per100 - 0.4) * 100 / 2.8), 
            fl: 100 - Math.min(99.99, (Math.max(0, p.defense.fl_pct - 0.08) * 100 / 0.24)),
        },
        rebounding: {
            contested_rebounding: Math.max(0.01, p.rebounding.c_rb_pct - 0.1) * (100 / 0.4) * minsAdjust, // normalize 0 to 0.8
            oreb: Math.max(0.01, p.rebounding.oreb_per100 - 0.4) * 100 / 6, 
            dreb: Math.max(0.01, p.rebounding.dreb_per100 - 3) * 100 / 10,
        },
        playmaking: {
            ast: Math.max(0.01, p.playmaking.ast_per100 - 2.6) * 100 / 10, 
            ast_rate: p.playmaking.ast_rate * 100 / 28, 
            tov: 100 - Math.min(99.99, (Math.max(0, p.playmaking.tov_rate - 4) * 100 / 20))
        },
        clutch: {
            end_three: Math.min(100, Math.max(0.001, p.clutch.close_end.fg3_pct - 0.15) * 100 / 0.45
                * Math.min(0.4, p.clutch.close_end.fg3a) / 0.4) * minsAdjust, 
            end_buff: Math.min(100, Math.max(0.001, p.clutch.close_end.fg_pct - 0.15) * 100 / 0.5
                * Math.min(1, p.clutch.close_end.fga) / 1) * minsAdjust,
            end_ft: Math.min(100, Math.max(0, p.clutch.close_end.ft_pct - 0.6) * 100 / 0.4) * minsAdjust,
            final_three: Math.min(100, Math.max(0.001, p.clutch.close_final.fg3_pct - 0.15) * 100 / 0.45
                * Math.min(0.2, p.clutch.close_end.fg3a) / 0.2) * minsAdjust, 
            final_buff: Math.min(100, Math.max(0.001, p.clutch.close_final.fg_pct - 0.15) * 100 / 0.5
                * Math.min(0.4, p.clutch.close_end.fg3a) / 0.4) * minsAdjust,
            final_ft: Math.min(100, Math.max(0, p.clutch.close_final.ft_pct - 0.6) * 100 / 0.4) * minsAdjust,
        }
    };
}

export function getOverall(r: any) {
    return r.impact * 0.06 + r.scoring.overall * 0.1
      + r.scoring.efficiency * 0.12 + r.scoring.three * 0.1
      + r.scoring.zones.restricted_area.skill * 0.05 + r.scoring.zones.restricted_area.ast_buff * 0.012 + r.scoring.zones.paint_non_ra.skill * 0.05 + r.scoring.zones.paint_non_ra.ast_buff * 0.012
      + r.scoring.zones.mid_range.skill * 0.05 + r.scoring.zones.mid_range.ast_buff * 0.012
      + r.scoring.zones.left_corner_3.skill * 0.03 + r.scoring.zones.left_corner_3.ast_buff * 0.01 + r.scoring.zones.right_corner_3.skill * 0.03 + r.scoring.zones.right_corner_3.ast_buff * 0.01
      + r.scoring.zones.above_break_3.skill * 0.05 + r.scoring.zones.above_break_3.ast_buff * 0.012
      + r.defense.overall * 0.1
      
      + r.defense.restricted_area * 0.04 + r.defense.paint_non_ra * 0.04
      + r.defense.mid_range * 0.04 + r.defense.three * 0.04
      + r.defense.stl * 0.08
      + r.defense.blk * 0.08
      + r.defense.fl * 0.02

      + r.rebounding.contested_rebounding * 0.04
      + r.rebounding.oreb * 0.1
      + r.rebounding.dreb * 0.08
      + r.playmaking.ast * 0.1 
      + r.playmaking.tov * 0.08
      + r.clutch.end_three * 0.006 + r.clutch.end_buff * 0.01 + r.clutch.end_ft * 0.002
      + r.clutch.final_three * 0.004 + r.clutch.final_buff * 0.006 + r.clutch.final_ft * 0.001;
}